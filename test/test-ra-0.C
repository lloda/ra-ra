...
template <class A>
void CheckArrayOutput(A const & a, real * begin)
{
    std::ostringstream o;
    o << a;
    cout << "a: " << o.str() << endl;
    ...
    assert(std::equal(ibegin, iend, begin));
}

template <class A>
void CheckArrayIO(A const & a, real * begin)
{
    std::ostringstream o;
    o << a;
    cout << "a: " << o.str() << endl;
    ...
    }
    {
        ...
        assert(std::equal(as.begin(), as.end(), bs.begin()));
        assert(std::equal(a.begin(), a.begin(), b.begin()));
    }
}
...
void CheckReverse(A && a)
{
    ...
    real check0[24] = { 17, 18, 19, 20,   21, 22, 23, 24,
                        ...
                        1, 2, 3, 4,     5, 6, 7, 8 };
    assert(std::equal(check0, check0+24, b0.begin()));
    ...
    assert(std::equal(check1, check1+24, b1.begin()));
    ...
    assert(std::equal(check2, check2+24, b2.begin()));
}

template <class A>
void CheckTranspose1(A && a)
{
    std::iota(a.begin(), a.end(), 1);
    cout << "a: " << a << endl;
    ...
    assert(std::equal(b.begin(), b.end(), check));
}
...
// @BUG strange that {{3,2},{2,1}} doesn't work...
        ...
        }
        {
            ...
            assert(a.dim[0].size==10);
            assert(a.dim[1].size==2);
...
// ra::Raw<T, 0> contains a pointer to T plus the dope vector of type Small<Dim, 0>. But after I put the data of Small in Small itself instead of in SmallBase, sizeof(Small<T, 0>) is no longer 0. That was specific of gcc, so better not to depend on it anyway...
    ...
    section("iterator");
    {
        ...
        assert(std::equal(pool, pool+6, r.begin()));
    }
    {
        real chk[6] = { 0, 0, 0, 0, 0, 0 };
        real pool[6] = { 1, 2, 3, 4, 5, 6 };
...
// STL-type iterators.
    {
        ...
        assert(std::equal(cpool, cpool+6, spool));
        cout << endl;
    }
    section("storage types");
    {
        ...
        assert(std::equal(pool, pool+6, s.begin()));

        ra::Unique<real> u({3, 2}, pool, pool+6);
        cout << "u rank: " << u.rank() << endl;
        assert(std::equal(pool, pool+6, u.begin()));
    ...
    section("copy between arrays, construct from iterator pair");
    {
        ...
        assert(std::equal(check, check+6, z.begin()));
    }
// In this case, the Raw + shape provides the driver.
    section("construct Raw from shape + driverless xpr");
    {
...

        {
            ...
            cout << b << endl;
            assert(std::equal(checkb, checkb+6, b.begin()));
        }
...
// @TODO Could this be made to work?
    ...
    section("construct Raw from shape + xpr");
    {
        ...
        cout << a << endl;
        assert(std::equal(checka, checka+6, a.begin()));
    ...
    section("construct Unique from Unique");
    {
        ...
        cout << "c: " << c << endl;
        assert(std::equal(check, check+6, c.begin()));
        ra::Unique<real, 2> d(std::move(a)); // ok; from fake temp
        cout << "d: " << d << endl;
        assert(std::equal(check, check+6, d.begin()));
    }
...
// b.rank() is runtime, so b()==44. and the whole assert argument become array xprs when ra-operators.H is included.
        assert(b.rank()==0 && b.size()==1 && b()==44.);
        b = 55.;
        cout << "b: " << b << endl;
        assert(b.rank()==0 && b.size()==1 && b()==55.); // see above for b.rank().
    }
...
// See note in (170).
    ...
    section("constructors from data in initializer_list");
    {
        real checka[6] = { 2, 3, 1, 4, 8, 9 };
        {
            ...
            assert(a.dim[0].size==2 && a.dim[1].size==3);
            assert(std::equal(a.begin(), a.end(), checka));
        }
        {
            ra::Unique<real> a { 2, 3, 1, 4, 8, 9 };
            assert(a.size()==6 && a.rank()==1);
            assert(std::equal(a.begin(), a.end(), checka));
            ra::Unique<real> b ({ 2, 3, 1, 4, 8, 9 });
            assert(b.size()==6 && b.rank()==1);
            assert(std::equal(b.begin(), b.end(), checka));
        }
        {
    ...
    section("row-major assignment from initializer_list, rank 2");
    {
        ...
        a = { 2, 3, 1, 4, 8, 9 };
        assert(a(0, 0)==2 && a(0, 1)==3 && a(1, 0)==1 && a(1, 1)==4 && a(2, 0)==8 && a(2, 1)==9);
        auto b = transpose(a, {1, 0});
        b = { 2, 3, 1, 4, 8, 9 };
        assert(b(0, 0)==2 && b(0, 1)==3 && b(0, 2)==1 && b(1, 0)==4 && b(1, 1)==8 && b(1, 2)==9);
        assert(a(0, 0)==2 && a(0, 1)==4 && a(1, 0)==3 && a(1, 1)==8 && a(2, 0)==1 && a(2, 1)==9);
        auto c = transpose(a, {1, 0});
        assert(a.data()==c.data() && a.size(0)==c.size(1) && a.size(1)==c.size(0));
        assert(every(b==c));
    }
    section("row-major assignment from initializer_list, rank 1");
    {
        ...
        a = { 2, 3, 1, 4, 8 };
        assert(a(0)==2 && a(1)==3 && a(2)==1 && a(3)==4 && a(4)==8);
    }
    section("subscripts");
    {
        section("Raw fixed rank");
        {
            ...
            assert(std::equal(r0.begin(), r0.end(), rcheck0));
            ra::Small<int, 0> i0 {};
            cout << "i0 rank: " << i0.rank() << endl;
            auto r0a = r.at(ra::Small<int, 0> {});
            assert(std::equal(r0a.begin(), r0a.end(), rcheck0));

            real rcheck1[2] = { 2, 5 };
            auto r1 = r(1);
            cout << "r1" << endl;
            std::copy(r1.begin(), r1.end(), std::ostream_iterator<real>(cout, " ")); cout << endl;
            assert(std::equal(r1.begin(), r1.end(), rcheck1));
            auto r1a = r.at(ra::Small<int, 1> {1});
            assert(std::equal(r1a.begin(), r1a.end(), rcheck1));
            ...
            // assert(std::equal(r2.begin(), r2.end(), rcheck2));
            cout << r2 << endl;
            assert(r2==5);

            auto r2a = r.at(ra::Small<int, 2> {1, 1});
            assert(std::equal(r2a.begin(), r2a.end(), rcheck2));
        }
        ...
        section("Raw fixed rank, unbeatable subscripts");
        {
            ...
            assert(ai.size()==i.size() && ai[0]==a[i[0]] && a[i[1]] && a[i[2]]);
            a(i) = ra::Unique<real, 1> {7, 8, 9};
            cout << a << endl;
            assert(a.size()==4 && a[0]==1 && a[i[0]]==7 && a[i[1]]==8 && a[i[2]]==9);
        }
...
// @TODO Make sure that this is real & = 99, etc. and not Raw<real, 0> = 99, etc.
        section("assign to rank-0 result of subscript");
        {
            ...
            assert(std::equal(check, check+6, a.begin()));
        }
    }
    section("construct from shape");
    {
        ...
        assert(sa[0]==3 && sa[1]==2 && sa[2]==4);
        real check[24];
        std::iota(check, check+24, 0);
        std::equal(check, check+24, a.begin());
    }
    section("Var rank Raw from fixed rank Raw");
    {
        ...
        assert(a.rank()==b.rank() && a.size(0)==b.size(0) && a.size(1)==b.size(1) && a.size(2)==b.size(2));
        assert(every(a==b));
    }
    section("I/O");
    {
...
// See note in (170).
        ...
        section("8");
        {
            ...
            CheckArrayOutput(a, check);
// default strides.
            ra::Raw<real> b { {2, 2, 2}, rpool };
            CheckArrayOutput(b, check);
        }
        section("9");
        {
...
// more cases with +2 candidates.
    ...
    section("ra::iota");
    {
        ...
        {
            assert(every(ra::iota(4)==ra::Owned<int, 1> {0, 1, 2, 3}));
            assert(every(ra::iota(4, 1)==ra::Owned<int, 1> {1, 2, 3, 4}));
            assert(every(ra::iota(4, 1, 2)==ra::Owned<int, 1> {1, 3, 5, 7}));
        }
...
// Cf [trc-01] in test-ra-compatibility.C.
        section("[tr0-01] frame-matching, forbidding unroll");
        {
            ...
            cout << b << endl;
            assert(every(b(0)==1) && every(b(1)==2) && every(b(2)==3));
        }
    }
    section("reverse array types");
    {
        CheckReverse(ra::Unique<real>({ 3, 2, 4 }, ra::default_init));
        CheckReverse(ra::Unique<real, 3>({ 3, 2, 4 }, ra::default_init));
    }
    section("transpose");
    {
        CheckTranspose1(ra::Unique<real>({ 3, 2 }, ra::default_init));
        CheckTranspose1(ra::Unique<real, 2>({ 3, 2 }, ra::default_init));
// can do these only with var-rank. @TODO Static-axes version of transpose.
        {
            ...
            cout << "b: " << b << endl;
            assert(b.rank()==1 && b.size()==2 && b[0]==1 && b[1]==4);
        }
        {
            ...
            auto b = transpose(a, ra::Small<int, 2> { 0, 0 });
            cout << "b: " << b << endl;
            assert(b.rank()==1 && b.size()==2 && b[0]==1 && b[1]==5);
        }
    }
    cout << "\nok." << endl;
    return 0;
