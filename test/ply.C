...
// TODO See Expr::CAN_DRIVE in expr.H. Doesn't generally work with Unique<RANK_ANY> because Expr needs to pick a driving argument statically. However, it does work when there's only one argument, since ply_ravel() & ply_index() are rank-dynamic.
        {
            auto test = [&](auto && a)
                {
                    ra::ply_index(ra::expr(print, a.iter())); cout << endl;
