...
#define TEST(CHECK_RANK_S)                                              \
    ...
    tr.section("explode for Small");
    {
        ...
        tr.info(ra::mp::print_int_list<lens> {}).test(std::is_same_v<ra::mp::int_list<2>, lens>);
        tr.info(ra::mp::print_int_list<steps> {}).test(std::is_same_v<ra::mp::int_list<1>, steps>);
