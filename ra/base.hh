...
template <class S, class T, class SS=S> using complement_list = complement_list_<S, T, SS>::type;
...
struct complement_list_
{
    using type = decltype([]{
        if constexpr (0==len<T>) {                                  // end of T.
            ...
            return complement_list<SS, rest<T>> {};
        } else {                                                    // keep searching in S.
            return complement_list<rest<S>, T, SS> {};
        }
    }());
};
...
struct combs_
{
    static_assert(K>=0);
    using re = rest<A>;
    using type = append<mapcons<first<A>, combs<re, K-1>>, combs<re, K>>;
};
...
size(V const & v)
{
    if constexpr (ANY!=size_s(v)) {
        ...
        dim_t s=1; for (rank_t k=0; k<rank(v); ++k) { dim_t l=v.len(k); if (l<0) return l; else s*=l; } return s;
    }
}
